...
local function removeSeatIfNotInVehicle(seat)
    ...
    while parent and parent ~= workspace do
        if parent:IsA("Model") then
            ...
            -- ถ้า Model มี DriveSeat อยู่ ถือว่ายานพาหนะ
...
-- ตรวจสอบ Seat ใหม่ที่ถูกเพิ่มเข้ามาแบบ Real-Time
...
local DesyncButton = MainTab:Button({
    ...
    Callback = function()
        ...
        local function CreatePartUnderFeet(character)
            ...
            -- รอให้ Character อยู่ใน workspace
            ...
            -- สร้าง Attachment ใน HRP และ Part
...
-- ประกาศตัวแปรทั้งหมดไว้ข้างนอก
local Character = Player.Character or Player.CharacterAdded:Wait()
local Humanoid
local RootPart
local EnabledSpeed = false
local SpeedValue = 34 -- ค่าความเร็วคงที่

-- Function สำหรับ setup character
local function SetupCharacter(char)
    Character = char
    Humanoid = char:WaitForChild("Humanoid")
    RootPart = char:WaitForChild("HumanoidRootPart")

    -- รีเซ็ตค่าเมื่อ spawn ใหม่
...
local Players = game:GetService("Players")
local player = Players.LocalPlayer
local dropfolder = workspace:WaitForChild("DroppedItems") -- เปลี่ยนชื่อตามของคุณ
local RunService = game:GetService("RunService")
...
-- ⚙️ ตัวแปรควบคุมเปิดปิด
_G.AutoPickItems = false

-- 🧲 ฟังก์ชันหลัก
task.spawn(
    function()
        while task.wait(0) do -- ตรวจทุก 0.2 วิ (เร็วแต่ไม่หน่วง)
...
local function HideName(char)
    ...
    if Hide then
        -- ซ่อนชื่อ
        if gui then
            gui.Enabled = false -- แนะนำให้ซ่อนแทนการลบ
        end
    else
        -- เปิดชื่อ
        if gui then
...
-- เมื่อเกิดใหม่ให้ซ่อนตาม toggle
...
-- เก็บค่าเดิมของปืนเพื่อใช้ restore
local OriginalWeaponStats = {}

-- ฟังก์ชันหาปืนที่กำลังถืออยู่ใน Character
local function GetEquippedTool()
    ...
    -- หา Tool ที่ถืออยู่ใน Character
...
-- ฟังก์ชันหา attribute ที่ตรงกับค่าเดิม
...
-- ฟังก์ชันแก้ไข attributes ของปืน
local function ModifyWeapon()
    ...
    -- บันทึกค่าเดิมถ้ายังไม่เคยบันทึก
    ...
    -- แก้ไข Accuracy (ค้นหาจากค่าเดิม 0.75)
    ...
    -- แก้ไข Automatic (ถ้าเปิด ให้เปิดทุก boolean attributes)
    ...
    -- แก้ไข fire_rate (ค้นหาจากค่าเดิม 370)
    ...
    -- แก้ไข range (ค้นหาจากค่าเดิม 80)
    ...
    -- แก้ไข recoil (ค้นหาจากค่าเดิม 0.25)
    ...
    -- แก้ไข reload_time (ค้นหาจากค่าเดิม 2)
...
-- ปุ่มตรวจสอบ Tool ที่ถืออยู่ (อัพเดท Desc แบบ real-time)
...
-- Auto-update เมื่อเปลี่ยนปืน (ถ้ายังเปิดโหมด mod อยู่)
local player = game.Players.LocalPlayer

-- ฟังก์ชันตรวจสอบและ mod tool ที่เพิ่งถือ
local function OnToolEquipped(tool)
    if tool:IsA("Tool") and WeaponSettings.IsModEnabled then
        task.wait(0.1) -- รอให้ tool โหลดเสร็จ
        OriginalWeaponStats = {} -- รีเซ็ตค่าเดิม
        ModifyWeapon()
        WindUI:Notify(
            {
                Title = "Weapon Mod",
...
-- เชื่อมต่อ event เมื่อมี tool เพิ่มเข้ามาใน Character
if player.Character then
    player.Character.ChildAdded:Connect(OnToolEquipped)
end

-- เชื่อมต่อใหม่ทุกครั้งที่ respawn
player.CharacterAdded:Connect(
    function(char)
...
-- เริ่มทำงานฟังก์ชันใน background
...
MiscTab:Toggle(
    {
        ...
        Callback = function(Value)
            ...
            -- ฟังก์ชันดึง items ทั้งหมดจาก UI
